...
def k_draw_box(img_plot, idx, pos):
    w = img_plot.width
    h = img_plot.height
    sz = 3
    draw = ImageDraw.Draw(img_plot)
    xc = round(w * pos[0].item())
    yc = round(h * pos[1].item())
    wbox2 = round(0.5 * w * pos[2].item())
    hbox2 = round(0.5 * h * pos[3].item())
    draw.rectangle((xc - sz, yc - sz, xc + sz, yc + sz), width = 1, outline=255, fill=255)
    draw.rectangle((xc - wbox2, yc - hbox2, xc + wbox2, yc + hbox2), width = 1, outline=255)
...
def get_count_errs(
    ...
    real_tensor = samples.tensors # 実際のテンソルを取り出す
    sz = real_tensor[0].shape
    w = sz[2]
    h = sz[1]
    ...
    for sample_ind in range(len(targets)):
        ...
        print('sample_boxes:', sample_boxes)
        ...
        print('sample_logits 2:', sample_logits)
        print('sample_boxes 2:', sample_boxes)
        ...
        rng = vmax - vmin
        print(vmin, vmax, rng)
        timg = (img - vmin) / rng # 0.0 to 1.0
        timg_u8 = (timg.cpu() * 128).to(torch.uint8)
        img_plot = torchvision.transforms.functional.to_pil_image(timg_u8)
        img_plot.save('./zzz/tmp.jpg')
        k = 0
        for box in sample_boxes:
            k_draw_box(img_plot, k, box)
            k += 1
        img_plot.save('./zzz/tmp_plot.jpg')
        #----------------------------------------------------------------
        ...
        print(args.crop, pred_cnt, args.num_select)
        ...
        print('args.simple_crop, pred_cnt, args.num_select, args.sam_tt_norm, args.exemp_tt_norm')
        print(args.simple_crop, pred_cnt, args.num_select, args.sam_tt_norm, args.exemp_tt_norm)
...
def k_disp(obj, sp):
    if type(obj) is dict:
        ...
        if type(obj) is torch.Tensor:
            print(sp, 'val: <torch.Tensor>', obj.shape)
        else:    
            print(sp, 'val:', type(obj))
...
def evaluate(
    ...
    for samples, targets in metric_logger.log_every(
        ...
        w = sz[2]
        h = sz[1]
        print('real_tensor.shape:', real_tensor.shape) # [Batch, Channel, Height, Width]  torch.Size([1, 3, 800, 1200])
        print('real_tensor[0].shape:', real_tensor[0].shape) # 0番目の画像にアクセス   torch.Size([3, 800, 1200])        
        rgb = real_tensor[0]
        plt.imsave('./zzz/rgb_0.jpg', rgb[0].numpy(), cmap='gray')
        plt.imsave('./zzz/rgb_1.jpg', rgb[1].numpy(), cmap='gray')
        plt.imsave('./zzz/rgb_2.jpg', rgb[2].numpy(), cmap='gray')
        print('type(rgb[0]):', type(rgb[0])) # type(rgb[0]): <class 'torch.Tensor'>
        print('rgb[0].dtype:', rgb[0].dtype) # rgb[0].dtype: torch.float32
        numpy_array0 = rgb[0].numpy() * 255
        numpy_array1 = rgb[1].numpy() * 255
        numpy_array2 = rgb[2].numpy() * 255
        ir  = numpy_array0.astype('uint8')
        ig  = numpy_array1.astype('uint8')
        ib  = numpy_array2.astype('uint8')
        rgb_image = np.dstack((ir ,ig, ib))
        plt.imsave('./zzz/rgb.jpg', rgb_image)
        """
        # ----------------------------------------------------------------------------
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            z = [torch.tensor([0]).to(device) for _ in targets]
            print('                  >>>  targets:', z)    
            ...
            print('args.box_threshold:', args.box_threshold)
            print('args.text_threshold:', args.text_threshold)
